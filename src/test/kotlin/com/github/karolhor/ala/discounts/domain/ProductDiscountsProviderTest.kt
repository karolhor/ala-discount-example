...
class ProductDiscountsProviderTest {
    ...
    private val baseDiscountEntity = ProductDiscountThresholdEntity(
        productId = productId,
    ...
    @Test
    fun `should convert fixed discount entries to proper domain object`() = runTest {
        // given
    ...
    @Test
    fun `should ignore entity when value is 0`() = runTest {
        // given
        val invalidEntity = baseDiscountEntity.copy(
    ...
    @Test
    fun `should convert quantity discount entries to proper domain object`() = runTest {
        // given
        val thresholdOne = baseDiscountEntity.copy(
            policy = DiscountPolicy.QUANTITY,
            min = 13,
            max = 15,
            value = "5.00".toBigDecimal(),
        )
        val thresholdTwo = baseDiscountEntity.copy(
            policy = DiscountPolicy.QUANTITY,
            min = 3,
            max = 12,
            value = "2.00".toBigDecimal(),
        )
        val thresholdThree = baseDiscountEntity.copy(
            policy = DiscountPolicy.QUANTITY,
            min = 16,
            max = null,
        ...
        val thresholdFour = baseDiscountEntity.copy(
            policy = DiscountPolicy.QUANTITY,
            min = null,
            max = 5,
            value = "1.00".toBigDecimal(),
        )
        coEvery { discountRepository.findDiscountsByProductId(productId) } returns flowOf(
            thresholdOne,
            thresholdTwo,
            thresholdThree,
            thresholdFour
    ...
    @Test
    fun `should provide and convert multiple discounts`() = runTest {
        // given
        val quantityDiscountEntity = baseDiscountEntity.copy(
