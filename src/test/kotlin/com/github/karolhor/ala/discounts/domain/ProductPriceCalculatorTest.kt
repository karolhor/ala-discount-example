...
class ProductPriceCalculatorTest {
    ...
    private val baseResult = TotalPrice(
    ...
    @Test
    fun `should return total price without discount when their not given`() = runTest {
        // given
    ...
    @Test
    fun `should return total price with discount`() = runTest {
        // given
        val discountStrategy = mockk<DiscountCalcStrategy>()
        val discount = mockk<ProductDiscount>()
        val mockedPriceDiscount = PriceDiscount(
        ...
        val expectedResult = baseResult.copy(
    ...
    @Test
    fun `should return total price with max available discount when multiple found for given product`() = runTest {
        // given
        ...
        val lowerPriceDiscount = PriceDiscount(
            rate = "3.10".toBigDecimal(),
            amount = "15.49".toBigDecimal(),
        )
        val greaterPriceDiscount = PriceDiscount(
            rate = "7.00".toBigDecimal(),
            amount = "31.80".toBigDecimal(),
        )

        coEvery { productDiscountsProvider.findDiscountsByProductId(product.id) } returns listOf(lowerValueDiscount, greaterValueDiscount)
